...
def signup():
    if request.method == 'POST':
        ...
        if not re.match(EMAIL_REGEX, email):
            ...
            flash("Invalid username. Must start with a letter and contain only letters, numbers, and underscores (5–30 characters).", "danger")
        else:
            db = current_app.config['DB_CONNECTION']
            if User.find_by_email(db, email):
                flash('Email is already registered.', 'danger')
...
def update_phone():
    ...
    if request.method == "POST":
        ...
        db.users.update_one({"email": current_user.email}, {
            "$set": {
                "username": username,
                "phone_no": phone_no,
                "full_name": full_name
